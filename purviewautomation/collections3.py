...
def _send_api_request(
    ...
    """Send a request to the Purview API

    Args:
        url (str): The URL to send the request to
        headers (Dict[str]): The headers to send with the request
    ...
    Returns:
        requests.Response: The response from the Purview API
    """
    response = requests.request(method=method, url=url, headers=headers, json=body)
    if response.status_code != 200:
        if response.status_code == 403:
            err_msg = (
                "Not authorized. The Service Principal or user would need to be granted proper permission "
                ...
                "The collection names are case sensitive. Mycollection1 is different than mycollection1."
                "For more info see: https://learn.microsoft.com/en-us/azure/purview/catalog-permissions."
            )
...
class PurviewCollections:
    ...
    def _get_only_collection_names(self, api_version: str = None):
        ...
        collections_request = _send_api_request(method="GET", url=url, headers=self.header)
    ...
    def list_collections(self, only_names: bool = False, pretty_print: bool = False, api_version: Optional[str] = None):
        ...
        url = f"{self.collections_endpoint}?api-version={api_version}"
        collection_request = _send_api_request(method="GET", url=url, headers=self.header)
        collections = collection_request["value"]

        if pretty_print:
            pprint(collections, sort_dicts=False)

        if only_names:
            coll_names = self._get_only_collection_names(api_version=api_version)
            if pretty_print:
                pprint(coll_names, sort_dicts=False)
            return coll_names
    ...
    def create_collections(
        ...
        **kwargs,
        ...
        coll_dict = self._get_only_collection_names(api_version=api_version)
        start_collection = self.get_real_collection_name(start_collection, api_version, force_actual_name)

        collection_list = []
        collection_names = _check_collection_names_type(collection_names=collection_names)
        ...
        for colls in collection_list:
            ...
            for index, name in enumerate(updated_collection_list):
                if index == 0:
                    ...
                    request = _send_api_request(method="PUT", url=url, headers=self.header, body=data)
